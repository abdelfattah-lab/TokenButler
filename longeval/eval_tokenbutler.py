...
def get_producer_layers(model):
    """
    Return all attention blocks that own a TokenButler predictor.
    Architecture-agnostic: relies only on class name ending in
    'AttentionExperimental' and the `is_predictor_owner` flag
    (plus a legacy fallback on layer_idx == 0).
    """
    producers = []
    for m in model.modules():
        # Works for LLaMA, Qwen, Mistral, etc as long as the class
        # name ends with 'AttentionExperimental'.
        if not m.__class__.__name__.endswith("AttentionExperimental"):
            continue

        if getattr(m, "is_predictor_owner", False):
            producers.append(m)
        elif (
            not hasattr(m, "is_predictor_owner")
            and getattr(m, "layer_idx", None) == 0
        ):
            producers.append(m)

    # Sort by layer_idx to keep ordering stable & consistent with training
    producers.sort(key=lambda m: getattr(m, "layer_idx", 0))
    return producers
