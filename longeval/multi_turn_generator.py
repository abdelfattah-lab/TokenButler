...
class Generator:
    ...
    def generate(self, input_ids, max_new_tokens=128, attention_mask=None):
        ...
        for i in range(max_new_tokens):
            ...
            self.attention_mask = torch.cat(
                [self.attention_mask, torch.ones_like(next_token, device=self.device)],
                dim=-1,
            )
    ...
    def generate_from_prompt(self, input_ids, max_new_tokens=128):
        ...
        # Prefill the prompt and initialize attention mask
        ...
        for _ in range(max_new_tokens - 1):
            # Expand attention mask to cover the new token being passed in
            self.attention_mask = torch.cat(
                [self.attention_mask, torch.ones_like(next_token, device=self.device)],
                dim=-1,
            )
