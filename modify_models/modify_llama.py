        ...
        **kwargs,
        ...
        if self.inference_mode:
            ...
            if self.flash_attn:
                ...
                if self.layer_idx > 0:
                    # Predictor logits inputs: [BH, Lq, D'] and [BH, Lk, D']
                    q_importance_tensor = self.producer.q_importance[:, self.layer_idx % self.producer_frequency, :, :].float().to(query_states.device)  # [BH, Lq, D']
                    k_importance_tensor = self.producer.k_importance[:, self.layer_idx % self.producer_frequency, :, :].float().to(key_states.device)    # [BH, Lk, D']
    
                    BH, Lq_imp, _ = q_importance_tensor.shape
                    _,  Lk_imp, _ = k_importance_tensor.shape
                    assert Lq_imp == q_len and Lk_imp == key_len
                    assert BH == bsz * self.num_heads
    
                    # Reshape to [B, H, L, D']
                    q_imp = q_importance_tensor.view(bsz, self.num_heads, q_len, self.dDash)
                    k_imp = k_importance_tensor.view(bsz, self.num_heads, key_len, self.dDash)
                    ...
                    Q_pred_sub = q_imp[:, :, loss_row_idx, :]           # [B,H,R,D']
                    K_pred_full = k_imp                                 # [B,H,Lk,D']
                    Q_pred_flat = Q_pred_sub.reshape(bsz * self.num_heads, R, self.dDash)
                    K_pred_flat = K_pred_full.reshape(bsz * self.num_heads, key_len, self.dDash)
