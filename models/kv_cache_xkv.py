...
class ShadowKVCache_xKey_CPU:
    ...
    def get_key_cache(self, layer_idx, position_ids, rope_func, cos_sin_cache):
        ...
        xkv.gather_copy_d2d_with_offsets(self.k_cache_buffer[layer_idx], self.offsets, self.cnts, self.batch_size, self.num_key_value_heads, int(self.sparse_budget*self.head_dim), self.kernel_offset, self.kernel_stride, self.select_sets)
...
class ShadowKVCache_xKV_CPU:
    ...
    def get_value_cache(self, layer_idx, position_ids, cos_sin_cache):
        ...
        # FIXME(max410011): Need a batch_gather_gemm cuda kernel
        xkv.gather_copy_d2d_with_offsets(self.v_cache_buffer[layer_idx], self.offsets, self.cnts, self.batch_size, self.num_key_value_heads, int(self.sparse_budget*self.head_dim), self.kernel_offset, self.kernel_stride, self.select_sets)
