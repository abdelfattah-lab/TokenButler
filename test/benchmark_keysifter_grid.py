...
# Suppress dynamo errors/warnings
...
# Use same context for fair comparison between baseline and KeySifter
PROMPT_LENGTH = 2**15  # 32K tokens (safe for full attention baseline)
GEN_LENGTH = 256  # Reduced for faster benchmarking
SPARSE_BUDGET = 8192
...
def main():
    ...
    print(colored("KeySifter Grid Search Benchmark", 'cyan', attrs=['bold']))
    print(colored("="*80, 'cyan'))

    # Use random initialization (no pretrained weights)
    # This allows testing all grid configurations without weight shape constraints
    WEIGHTS_PATH = ''

    # Create output file with timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    OUTPUT_FILE = f'test/benchmark_keysifter_grid_{timestamp}.csv'
    ...
    print(f"\n{colored('Configuration:', 'yellow')}")
    print(f"  Prompt Length: {PROMPT_LENGTH:,} tokens ({PROMPT_LENGTH//1024}K)")
    print(f"  Generation Length: {GEN_LENGTH:,} tokens")
    print(f"  Sparse Budget: {SPARSE_BUDGET:,}")
    print(f"  dDash values: {dDash_values}")
    print(f"  Intermediate dim values: {intdim_values}")
    print(f"  Producer frequency values: {prod_freq_values}")
    print(f"  Predictor weights: {colored('Random initialization (no pretrained weights)', 'yellow')}")
    print()
    ...
    # Prepare CSV
    fieldnames = [
        'attn_mode', 'dDash', 'intdim', 'producer_frequency',
        'sparse_budget', 'decode_throughput', 'memory_allocated_gb', 'prompt_length'
    ]
    ...
    for config in pbar:
        ...
        if result:
            row = {
                ...
                'sparse_budget': SPARSE_BUDGET if attn_mode == 'keysifter' else '',
            ...
            # Print progress
            print(f"  Throughput: {result['decode_throughput']:.2f} tok/s | Memory: {result['memory_allocated_gb']:.2f} GB")
        else:
            print(colored(f"\nFailed config: {desc}", "red"))

    abs_path = os.path.abspath(OUTPUT_FILE)
    print(f"\n{colored('='*80, 'green')}")
    print(colored("BENCHMARK COMPLETE", 'green', attrs=['bold']))
    print(f"{colored('Results saved to:', 'green')} {colored(abs_path, 'cyan', attrs=['bold'])}")
